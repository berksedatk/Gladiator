...
module.exports = {
  ...
  execute(bot, message, args) {
    ...
    if (!args[0] || !amount) return message.error("You must provide a number between 1 and 100.")
    if (amount >= 101) return message.channel.error("The range must be between 1 and 100.");

    message.delete().then(() => {
      if (!user && !args[1]) {
      message.channel.bulkDelete(amount).then(messages => {
        message.success(`Done! Cleaned \`${amount}\` messages for you!`).then(msg => msg.delete({timeout: 5000}))
      });
    } else if (user) {
      message.channel.messages.fetch({ limit: 100 }).then(async messages => {
        ...
        message.success(`Done! Cleaned \`${amount}\` messages of ${user.username} for you!`).then(msg => msg.delete({timeout: 5000}))
      });
      ...
      message.channel.messages.fetch({ limit: 100 }).then(async messages => {
        ...
        await message.channel.bulkDelete(messages).catch(error => console.log(error.stack));
        message.success(`Done! Cleaned \`${amount}\` messages that include invites for you!`).then(msg => msg.delete({timeout: 5000}))
      });
    } else if (args[1].toLowerCase() === "links") {
      message.channel.messages.fetch({ limit: 100 }).then(async messages => {
        messages = messages.filter(m => /(?:(?:https?|ftp):\/\/)?[\w/\-?=%.]+\.[\w/\-?=%.]+/g.test(m.content)).array().slice(0, amount);
        await message.channel.bulkDelete(messages).catch(error => console.log(error.stack));
        message.success(`Done! Cleaned \`${amount}\` messages that include links for you!`).then(msg => msg.delete({timeout: 5000}))
      });
    } else if (args[1].toLowerCase() === "attachments") {
      message.channel.messages.fetch({ limit: 100 }).then(async messages => {
        messages = messages.filter(m => m.attachments.size > 0).array().slice(0, amount);
        await message.channel.bulkDelete(messages).catch(error => console.log(error.stack));
        message.success(`Done! Cleaned \`${amount}\` messages that include attachments for you!`).then(msg => msg.delete({timeout: 5000}))
      });
      ...
      message.channel.messages.fetch({ limit: 100 }).then(async messages => {
        ...
        await message.channel.bulkDelete(messages).catch(error => console.log(error.stack));
        message.success(`Done! Cleaned \`${amount}\` messages that is longer than \`${length}\` characters for you!`).then(msg => msg.delete({timeout: 5000}))
      });
      ...
      message.channel.messages.fetch({ limit: 100 }).then(async messages => {
        messages = messages.filter(m => m.content.length < length).array().slice(0, amount);
        await message.channel.bulkDelete(messages).catch(error => console.log(error.stack));
        message.success(`Done! Cleaned \`${amount}\` messages that is shorter than \`${length}\` characters for you!`).then(msg => msg.delete({timeout: 5000}))
      });
    } else if (args[1]) {
      message.channel.messages.fetch({ limit: 100 }).then(async messages => {
        messages = messages.filter(m => m.content.toLowerCase().includes(args[1].toLowerCase())).array().slice(0, amount);
        await message.channel.bulkDelete(messages).catch(error => console.log(error.stack));
        message.success(`Done! Cleaned \`${amount}\` messages that include \`${args[1]}\` for you!`).then(msg => msg.delete({timeout: 5000}))
      });
    }
    })
  }
