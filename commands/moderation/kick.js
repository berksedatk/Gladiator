...
module.exports = {
  ...
  async execute(bot, message, args) {
    ...
    if (user.id === message.author.id) return message.channel.send("<:cross:724049024943915209> | You can't kick yourself, dum dum!");
    if (!message.guild.members.cache.get(user.id).kickable) return message.channel.send("<:cross:724049024943915209> | This user is too powerful for me.");
    if (message.guild.members.cache.get(user.id).roles.highest.position >= message.member.roles.highest.position && message.guild.owner.id != message.author.id) return message.channel.send(":x: | You can't kick this member, they are too powerful for you.");
