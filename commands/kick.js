      ...
      try {
        user.send(`You have been **kicked** in **${message.guild.name}** for the reason: \n${args.join(" ")}`).then(() => {
          user.kick(user, {reason: args.join(" ")}).then(() => {
            return message.channel.send(`Case ID: \`#${caseNumber}\` \n:white_check_mark: | **${user.user.tag}** has been kicked! **Reason:** ${reason}`);
          }).catch(err => {
            return message.channel.send("I couldn't kick the user, here is the error:" + err);
          });
        }).catch(err => {
          user.kick(user, {reason: args.join(" ")}).then(() => {
            return message.channel.send(`Case ID: \`#${caseNumber}\` \n:white_check_mark: | **${user.user.tag}** has been kicked and I couldn't inform the user that they are kicked, welp. That's not my case ¯\\\_(ツ)\_/¯ **Reason:** ${reason}`);
          }).catch(err => {
            return message.channel.send("I couldn't kick the user, here is the error:" + err);
          });
        });
