...
module.exports = {
  ...
  async execute(bot, message, args) {
    if (!args[0]) return message.channel.send("<:cross:724049024943915209> | You didn't provide a option or emoji.")

    if (args[0].toLowerCase() === "add" && message.member.hasPermission("MANAGE_EMOJIS")) {
      if (!args[1]) return message.channel.send("<:cross:724049024943915209> | You didnt't provide a url or a emoji to add.");

      if (validURL(args[1])) {
        let name;
        if (args[2]) name = args[2]
        message.guild.emojis.create(args[1], name ? name : `emoji_${message.guild.emojis.cache.size}`).then(e => {
          return message.channel.send("<:tick:724048990626381925> | Emoji has been created! " + e.toString())
        }).catch(err => {
        ...
        return message.channel.send("<:cross:724049024943915209> | You didn't provide a custom emoji or a image to make a emoji of.")
      }

    } else if (args[0].toLowerCase() === "remove" || args[0].toLowerCase() === "delete" && message.member.hasPermission("MANAGE_EMOJIS")) {
      if (!args[1])return message.channel.send("<:cross:724049024943915209> | You didn't provide a emoji.");
      let emojiID = args[1].lastIndexOf(":") != -1 ? args[1].slice(args[1].lastIndexOf(":") + 1, args[1].length - 1) : undefined
      if (!emojiID) return message.channel.send("<:cross:724049024943915209> | You didn't provide true a emoji.");

      let emoji = message.guild.emojis.cache.get(emojiID)
      if (!emoji) return message.channel.send("<:cross:724049024943915209> | This emoji is not from this server.");

      emoji.delete().then(() => {
        return message.channel.send("<:tick:724048990626381925> | Emoji has been removed from the server successfully.")
      }).catch(err => {
        return message.channel.send("An error occured: " + err)
      })
      ...
      if (!emojiID) return message.channel.send("<:cross:724049024943915209> | You didn't provide a true emoji.");
